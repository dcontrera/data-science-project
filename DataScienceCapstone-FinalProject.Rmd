...
ng <- ng %>% arrange(-freq)
ngn <- ng %>% group_by(len) %>% summarise(n = sum(freq))
ng$perc <- ng$freq
# ngtest <- ng %>% mutate(perc = perc / ngn$n[ngn$len == len])
# ngtest <- ng %>% mutate(perc = perc / ngn$n[ngn$len == len])
for (i in 1:length(ngn$len)) {
    ng$perc[ng$len == ngn$len[i]] <- ng$perc[ng$len == ngn$len[i]] / (ngn$n[i]**0.7) / (ngn$len[i]**1.3)
}
ng <- ng %>% arrange(-perc)
# ng$perc_cum <- ng$perc
...
predMark <- function (wi = "", hist, m) {
    ...
    preds <- unlist(apply(m, 1, which.max))
    predTops <- apply(m, 1, function (x) {data.frame(Pbo = x) %>% slice_max(n = 4, order_by = Pbo, with_ties = FALSE) %>% rownames()})
    pred <- data.frame(hist = hist[1:length(preds)], pred = colnames(m)[preds])
    for (i in 2:nrow(predTops)) {
        pred[ , paste("pred", i)] <- predTops[i, ]
    }
    pred
}
```
